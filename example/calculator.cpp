...
#define assert(expression)                                                     \
  ctest::mock_assert((long)(expression), #expression, __FILE__, __LINE__)
namespace ctest {
...
static BinaryOperator find_operator_function_by_string(
  ...
  for (i = 0; i < number_of_operator_functions; i++) {
    const OperatorFunction *const operator_function = &operator_functions[i];
    if (strcmp(operator_function->_operator, operator_string) == 0) {
 ...
 */
static int perform_operation(int number_of_arguments, char *arguments[],
                             const size_t number_of_operator_functions,
                             const OperatorFunction *const operator_functions,
